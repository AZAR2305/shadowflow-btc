# ShadowFlow BTC ↔ STRK OTC Bridge

![ShadowFlow](https://img.shields.io/badge/Status-Live%20on%20Testnet-brightgreen)
![Bitcoin](https://img.shields.io/badge/Bitcoin-Testnet4-orange)
![Starknet](https://img.shields.io/badge/Starknet-Sepolia-blue)
![License](https://img.shields.io/badge/License-MIT-green)

## 🎯 Project Overview

ShadowFlow is a **decentralized atomic swap bridge** enabling trustless peer-to-peer exchanges between Bitcoin and Starknet (STRK) tokens. Users can swap at real-time exchange rates with cryptographic guarantees of settlement.

It combines:
- **Intent-based matching**: Users create swap intents that are matched with complementary intents
- **ZK proofs**: Garaga-based cryptographic verification of intent validity
- **Starknet smart contracts**: On-chain verification and settlement execution
- **Escrow-style settlement**: Funds locked until both parties approve and rates validate
- **Optional TEE attestation**: SGX/Nitro trusted execution environment support

### Key Features
- ⚡ **Atomic Swaps**: Both transfers execute simultaneously or neither transfers
- 🔐 **Non-Custodial**: Users maintain full control of private keys
- 🔄 **Instant Settlement**: Real-time execution with on-chain verification
- 💰 **Fair Rates**: Pyth Network oracles with 5% tolerance
- 🌍 **Cross-Chain**: Bitcoin ↔ Starknet seamless bridge
- 📱 **Multi-Wallet**: Support for Xverse, Unisat (BTC) & Argent X, Braavos (STRK)

## 📦 Repository Structure

This is a full-stack monorepo with three main components:

```
shadowflow-BTC/
├── frontend/                      # Next.js 14 UI Application
│   ├── README.md                 # Frontend documentation
│   ├── components/               # React components
│   ├── hooks/                    # Custom hooks (wallet integration)
│   ├── lib/                      # Client utilities (btcClient, balanceFetcher)
│   ├── app/                      # Next.js app router
│   └── package.json              # Dependencies
│
├── backend/                       # Next.js API Routes & Server Logic
│   ├── README.md                 # Backend documentation
│   ├── app/api/otc/              # OTC intent & match endpoints
│   └── lib/server/               # Server utilities (matching, escrow, Garaga)
│
├── blockchain/                    # Smart Contracts & Deployment
│   ├── README.md                 # Blockchain documentation
│   ├── contracts/                # Cairo smart contracts
│   ├── scripts/                  # Deployment scripts
│   └── deployments/              # Deployment artifacts
│
├── .env                          # Environment configuration
├── package.json                  # Root dependencies
└── README.md                     # This file
```

**👉 See individual folder READMEs for detailed documentation:**
- [Frontend Guide](./frontend/README.md) - UI/UX & components
- [Backend Guide](./backend/README.md) - API & logic
- [Blockchain Guide](./blockchain/README.md) - Contracts & deployment

## 🔄 Complete User Flow (Atomic Swap Process)

### **STEP 1: User Creates Intent**

```
User A (Bitcoin Side)
    │
    ├─→ Opens http://localhost:3000/otc-intent
    │
    ├─→ Fills Form:
    │    - Send: 0.0001 BTC
    │    - Receive: 0.37 STRK
    │    - Wallet: bc1qxxx (Xverse)
    │
    ├─→ Clicks "Create Intent"
    │
    └─→ Signs message with Xverse wallet
         Message: "Sign this intent: [hash]"
         Signature: sig_xxxxx
```

### **STEP 2: Backend Validates & Creates Intent**

```
POST /api/otc/intents
{
  step: "validate",
  amount: 0.0001,
  sendChain: "btc",
  receiveChain: "strk",
  receiveWalletAddress: "0x074...",
  walletAddress: "bc1qxxx...",
  signature: "304402203f...",
  depositAmount: 0.0001,
  priceThreshold: 0.37
}
         │
         ├─→ [Backend Validation]
         │    • Amount > 0? ✓
         │    • Valid Bitcoin address (bc1q...)? ✓
         │    • Valid Starknet address (0x...)? ✓
         │    • Signature valid (ECDSA)? ✓
         │
         ├─→ [Fetch Real-Time Exchange Rates]
         │    Method: Pyth Network Oracle
         │    • BTC/USD Price: $42,000.50
         │    • STRK/USD Price: $11.32
         │    • Calculated Rate: 3,706.5 STRK/BTC
         │    • Confidence: ±0.5%
         │
         ├─→ [Calculate Required Receive Amount]
         │    receiveAmount = 0.0001 * 3,706.5
         │    = 0.37065 STRK
         │    (User threshold: 0.37 STRK) ✓
         │
         ├─→ [Generate Garaga ZK Proof]
         │    🔐 REAL KNOWLEDGE-PROOF GENERATION
         │    GaragaProver creates zero-knowledge proof:
         │    • Proves: Amount ≤ maximum allowed
         │    • Proves: Price within user tolerance
         │    • Proves: Signature authorizes this intent
         │    Without revealing: private data
         │    
         │    Proof Generated:
         │    {
         │      proofHash: "0x3f14a8b9c2d5e6f7a8b9...",
         │      commitment: "0x7a3f9c2e1b4d6f8h9i0j...",
         │      constraintCount: 847,
         │      proofSize: 2048 bytes,
         │      verified: true
         │    }
         │
         ├─→ [Create Intent in Database]
         │    {
         │      intentId: "intent_7a3f9c2e",
         │      wallet: "bc1qxxx",
         │      amount: 0.0001,
         │      sendChain: "btc",
         │      receiveChain: "strk",
         │      receiveAmount: 0.37065,
         │      status: "pending",
         │      createdAt: 2026-04-01T14:20:00Z,
         │      expiresAt: 2026-04-01T14:25:00Z,
         │      exchangeRate: 3706.5,
         │      garagaProof: "0x3f14a8b9c2d5e6f7a8b9..."
         │    }
         │
         └─→ Response: 200 OK
              {
                intentId: "intent_7a3f9c2e",
                messageToSign: "Sign intent: 0x7a3f9c2e...",
                hasMatch: false,
                proofHash: "0x3f14a8b9c2d5e6f7a8b9..."
              }
```

### **STEP 3: User A Funds Escrow**

```
User A clicks "Fund Escrow" on Swap Matching page
         │
         ├─→ [Sign escrow message with Xverse]
         │    Message: "ESCROW:intent_abc123:btc"
         │
         ├─→ [Backend receives signature]
         │    POST /api/otc/intents
         │    step: "execute"
         │
         ├─→ [Create transaction hash]
         │    Hash: 0x1a2b3c4d5e6f7g8h...
         │
         └─→ [Update Intent Status]
              status: "escrow_funding"
              fundedToEscrow: true
```

### **STEP 4: Matching Engine Finds User B**

```
Backend Matching Service (Polling every 2 seconds)
    │
    ├─→ Query all pending intents
    │
    ├─→ Find complement:
    │    User A: Send 0.0001 BTC → Receive 0.37 STRK
    │            ↓
    │    User B: Send 0.37 STRK → Receive 0.0001 BTC
    │    ✓ MATCH FOUND!
    │
    ├─→ Verify rates are within 5% tolerance:
    │    Exchange rate at match time: 3,700 STRK/BTC
    │    Stored rate: 3,717 STRK/BTC
    │    Difference: 0.46% ✓
    │
    ├─→ Verify Garaga ZK Proofs for Both Intents:
    │    🔐 ZERO-KNOWLEDGE PROOF VERIFICATION
    │    Backend verifies both proofs locally:
    │    
    │    User A's proof verification:
    │    ✓ Proof commitment valid
    │    ✓ Signature proof valid (ECDSA)
    │    ✓ Amount constraint satisfied
    │    ✓ Price constraint satisfied
    │    
    │    User B's proof verification:
    │    ✓ Proof commitment valid
    │    ✓ Signature proof valid (STARK)
    │    ✓ Amount constraint satisfied
    │    ✓ Price constraint satisfied
    │
    └─→ Lock match (can't be used by other users)
         matchId: "match_xyz789"
         status: "matched"
         garaga_verified: true
```

### **STEP 5: Both Users Sign Match**

```
User A & User B both receive notification:
    "Match found! Click to sign"
         │
         ├─→ User A signs with Xverse
         │    Message: "MATCH:match_xyz"
         │    Signature: sig_aaa
         │
         └─→ User B signs with ArgentX
              Message: "MATCH:match_xyz"
              Signature: sig_bbb
              
[Backend tracking]
status: "both_signed"
```

### **STEP 6: Both Users Fund Escrow**

```
┌─────────────────────┬─────────────────────┐
│   User A (BTC)      │   User B (STRK)     │
├─────────────────────┼─────────────────────┤
│  Funds Escrow with  │  Funds Escrow with  │
│   0.0001 BTC        │   0.37 STRK         │
│   sig_aaa           │   sig_bbb           │
│                     │                     │
│  ✓ ESCROW LOCKED    │  ✓ ESCROW LOCKED    │
└─────────────────────┴─────────────────────┘
              │              │
              └──────┬───────┘
                     │
          Both amounts in escrow ✓
         status: "escrow_funded"
```

### **STEP 7: Atomic Settlement (Execution)**

```
Backend Settlement Service
    │
    ├─→ Verify: Both escrow funded? ✓
    │
    ├─→ Call Starknet Escrow Contract
    │    contractAddress: 0x04ab814de65d6ce99bb8801b33fcd751ae21b28f1f4726363be93d12b99ecbf4
    │    method: execute_swap()
    │
    ├─→ Submit Garaga ZK Proofs to Verifier Contract:
    │    🔐 ON-CHAIN PROOF VERIFICATION (CRITICAL SECURITY STEP)
    │    Verifier Contract Address: 0x065071fc0289ffc7ce91dc4e4c65cd7216a9bc311e475b18758a30268fdb1801
    │    
    │    Both proofs submitted to Garaga Verifier:
    │    ├─ User A's proof: 2048 bytes STARK proof
    │    │  Verified constraints: Amount + Price + Authorization
    │    │
    │    ├─ User B's proof: 2048 bytes STARK proof
    │    │  Verified constraints: Amount + Price + Authorization
    │    │
    │    └─ Verifier responds: ✅ BOTH VALID
    │       If invalid → txn reverts, funds safe
    │       If valid → Execute settlement below
    │
    ├─→ Smart Contract Execution:
    │    ┌─────────────────────────────────────────┐
    │    │   STARKNET ESCROW SMART CONTRACT        │
    │    ├─────────────────────────────────────────┤
    │    │ // Garaga verified both proofs          │
    │    │ assert(garagaVerifier.verify(proofA))   │
    │    │ assert(garagaVerifier.verify(proofB))   │
    │    │                                         │
    │    │ // Verify both approvals                │
    │    │ assert(partyA.approved && partyB.approved)
    │    │                                         │
    │    │ // Transfer BTC (via bridge)            │
    │    │ Transfer 0.0001 BTC                     │
    │    │   from Escrow                           │
    │    │   to User B BTC wallet                  │ ← BTC arrives
    │    │                                         │
    │    │ // Transfer STRK (on-chain)             │
    │    │ Transfer 0.37 STRK                      │
    │    │   from Escrow                           │
    │    │   to User A STRK wallet                 │ ← STRK arrives
    │    │                                         │
    │    │ // Emit settlement event                │
    │    │ emit SwapExecuted(matchId, txHash)      │
    │    │                                         │
    │    │ // Mark settled to prevent replay       │
    │    │ settled[matchId] = true                 │
    │    └─────────────────────────────────────────┘
    │
    ├─→ Backend confirms transaction
    │    - Wait for L1 confirmation
    │    - Transaction hash: 0x1a2b3c4d5e6f7g8h...
    │    - Block number: 12345678
    │    - Gas used: 145,000
    │
    └─→ Mark status as "executed"
         Both users notified with txHash
         
    ✅ ATOMIC SWAP SETTLED & FINALIZED
```

### **STEP 8: Confirmation & Settlement Complete**

```
╔════════════════════════════════════════════════════════════╗
║  ✅ ATOMIC SWAP SUCCESSFULLY EXECUTED                      ║
╠════════════════════════════════════════════════════════════╣
║                                                            ║
║  Transaction Hash (Starknet):                              ║
║  0x073bf8a8c4b7a192e9f3c5d6e7a8b9c0d1e2f3a4b5c6d7       ║
║                                                            ║
║  Block Number: 12,345,678                                 ║
║  Timestamp: 2026-04-01 14:30:45 UTC                        ║
║  Gas Used: 145,230 wei                                     ║
║  Status: ✅ CONFIRMED                                     ║
║                                                            ║
║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━    ║
║                                                            ║
║  YOU SENT:       0.0001 BTC                                ║
║  YOU RECEIVED:   0.37 STRK                                 ║
║  Exchange Rate:  3,700 STRK per BTC                        ║
║  Settlement Fee: 0.375 STRK (0.1%)                         ║
║                                                            ║
║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━    ║
║                                                            ║
║  COUNTERPARTY DETAILS:                                     ║
║  Payment Address: 0x0749a72f41ac69...[shortened]           ║
║  Received From:   0.37 STRK (Confirmed)                    ║
║                                                            ║
║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━    ║
║                                                            ║
║  Escrow Contract: 0x04ab814de65d6ce99bb8801b33fcd751...   ║
║  Cross-Chain Bridge: Bitcoin Testnet4 → Starknet Sepolia  ║
║                                                            ║
║  [View on Starkscan] [Download Receipt] [New Swap]        ║
║                                                            ║
╚════════════════════════════════════════════════════════════╝
```

---

## 📊 Complete System Flow Diagram

```mermaid
sequenceDiagram
    participant UA as User A<br/>(Bitcoin)
    participant FE as Frontend<br/>(React)
    participant API as Backend API<br/>(Validation)
    participant MATCH as Matching<br/>Engine
    participant SC as Escrow<br/>Contract
    participant UB as User B<br/>(Starknet)

    rect rgb(200, 220, 255)
    Note over UA,UB: PHASE 1: Intent Creation
    UA->>FE: Create swap intent<br/>0.0001 BTC → 0.37 STRK
    FE->>UA: Sign message with Xverse
    UA->>FE: Signature generated
    FE->>API: POST /api/otc/intents<br/>(step: "validate")
    API->>API: Validate amount & signature<br/>Fetch exchange rate (Pyth)
    API->>FE: intentId + messageToSign
    FE-->>UA: Awaiting counterparty...
    end

    rect rgb(220, 255, 220)
    Note over UA,UB: PHASE 2: Intent Matching
    MATCH->>API: Poll for matchable intents
    API->>MATCH: Intent from User A found
    MATCH->>MATCH: Search for complement<br/>(0.37 STRK → 0.0001 BTC)
    MATCH->>API: Intent from User B found!
    MATCH->>MATCH: Verify rates within 5%<br/>Create match
    UB->>FE: Match notification received
    end

    rect rgb(255, 240, 200)
    Note over UA,UB: PHASE 3: Both Users Sign
    UA->>FE: Click sign match
    FE->>UA: Sign with Xverse
    UA->>API: Submit signature (User A)
    
    UB->>FE: Click sign match
    FE->>UB: Sign with ArgentX
    UB->>API: Submit signature (User B)
    
    API->>MATCH: Both signatures verified
    MATCH->>MATCH: status = "both_signed"
    end

    rect rgb(255, 220, 200)
    Note over UA,UB: PHASE 4: Escrow Funding
    UA->>FE: Fund escrow with 0.0001 BTC
    FE->>UA: Sign escrow funding
    UA->>SC: Deposit 0.0001 BTC
    SC->>SC: Lock funds (User A)
    
    UB->>FE: Fund escrow with 0.37 STRK
    FE->>UB: Sign escrow funding
    UB->>SC: Deposit 0.37 STRK
    SC->>SC: Lock funds (User B)
    
    API->>SC: Check both funded?
    SC-->>API: ✓ Both locked
    API->>MATCH: status = "escrow_funded"
    end

    rect rgb(200, 255, 200)
    Note over UA,UB: PHASE 5: Atomic Settlement
    API->>SC: Execute settlement
    SC->>SC: Transfer 0.0001 BTC to User B
    SC->>SC: Transfer 0.37 STRK to User A
    SC->>API: txHash + confirmation
    API->>MATCH: status = "executed"
    
    FE-->>UA: ✅ Swap complete!<br/>Transaction: 0x...
    FE-->>UB: ✅ Swap complete!<br/>Transaction: 0x...
    end
```

## 🏗️ System Architecture (Real Production)

```mermaid
graph TB
    subgraph Frontend["🎨 Frontend (React)"]
        A1["OTC Intent Page"]
        A2["Swap Matching UI"]
        A3["Wallet Integration"]
        A1 --> A2 --> A3
    end
    
    subgraph Wallets["💳 Wallets"]
        W1["Xverse/Unisat<br/>(Bitcoin)"]
        W2["ArgentX/Braavos<br/>(Starknet)"]
    end
    
    subgraph Backend["🔧 Backend API Routes"]
        B1["POST /api/otc/intents<br/>Validate & Create"]
        B2["GET /api/otc/matches<br/>Find Counterparty"]
        B3["Matching Engine<br/>Real-time Pairing"]
        B4["Settlement Handler<br/>Execute Swap"]
    end
    
    subgraph Services["🛠️ Services"]
        S1["Signature Verification"]
        S2["Pyth Price Oracle"]
        S3["Mempool API<br/>(Bitcoin)"]
        S4["ZK Proof Validator"]
    end
    
    subgraph Blockchain["⛓️ Smart Contracts"]
        SC1["Escrow Contract<br/>(Starknet)"]
        SC2["Token Transfers<br/>(Both Chains)"]
    end
    
    Frontend --> Wallets
    Frontend --> Backend
    Backend --> Services
    Backend --> Blockchain
    Services --> Blockchain
    
    style Frontend fill:#e1f5ff
    style Wallets fill:#fff3e0
    style Backend fill:#f3e5f5
    style Services fill:#e8f5e9
    style Blockchain fill:#fce4ec
```

## 🔄 Intent Status State Machine

```mermaid
stateDiagram-v2
    [*] --> pending: User creates intent
    
    pending --> matched: Matching engine<br/>finds counterparty
    
    pending --> cancelled: User cancels<br/>or timeout
    
    matched --> both_signed: Both users<br/>sign transaction
    
    matched --> failed: Exchange rate<br/>changed >5%
    
    both_signed --> escrow_funding: User A funds<br/>escrow
    
    both_signed --> escrow_partially: Only one user<br/>funded
    
    escrow_funding --> escrow_funded: User B funds<br/>escrow
    
    escrow_partially --> escrow_funded: Second user<br/>funds escrow
    
    escrow_funded --> executing: Automatic<br/>settlement begins
    
    executing --> executed: Atomic swap<br/>confirmed on-chain
    
    executing --> failed: Settlement<br/>failed (refund)
    
    failed --> refund_pending: Initiate refunds
    refund_pending --> [*]
    
    cancelled --> [*]
    executed --> [*]
    
    style pending fill:#fff9c4
    style matched fill:#ffe0b2
    style both_signed fill:#ffccbc
    style escrow_funding fill:#ffab91
    style escrow_funded fill:#ffab91
    style executing fill:#c8e6c9
    style executed fill:#a5d6a7
    style failed fill:#ef9a9a
    style cancelled fill:#bdbdbd
    style refund_pending fill:#ffccbc
```

## 💰 Exchange Rate Validation Flow

```mermaid
graph LR
    A["User A: 0.0001 BTC<br/>Desired: 0.37 STRK"] -->|Timestamp T0| B["Fetch Current Rate<br/>from Pyth Oracle"]
    B -->|BTC=$42,000<br/>STRK=$11.30| C["Calculate Rate<br/>3,717 STRK/BTC"]
    
    C -->|Timestamp T1| D["User B: 0.37 STRK<br/>Desired: 0.0001 BTC"]
    D -->|Fetch Current Rate| E["Validate Rate<br/>Tolerance Check"]
    
    E -->|3,700 STRK/BTC<br/>Δ = 0.46%<br/>✓ Within 5%| F["✅ Match Approved"]
    E -->|Out of bounds<br/>❌ > 5% change| G["❌ Match Rejected<br/>Request Quote"]
    
    F --> H["Proceed to<br/>Escrow Funding"]
    G --> I["User can retry<br/>or cancel"]
    
    style A fill:#e3f2fd
    style D fill:#e3f2fd
    style F fill:#c8e6c9
    style G fill:#ffcdd2
    style H fill:#81c784
    style I fill:#ffb74d
```

## 🔐 Settlement Security Checks

```mermaid
graph TD
    A["Settlement Initiated<br/>(Both Escrow Funded)"] --> B{Verify Signatures}
    
    B -->|Invalid| C["❌ Reject Settlement<br/>Refund Escrow"]
    B -->|Valid| D{Check Exchange Rate}
    
    D -->|Out of Tolerance| C
    D -->|Within 5%| E{Verify Amounts<br/>Match}
    
    E -->|Mismatch| C
    E -->|Correct| F{Check Escrow<br/>Balances}
    
    F -->|Insufficient| C
    F -->|Sufficient| G["Execute Atomic<br/>Transfer"]
    
    G --> H["Transfer User A Amount<br/>to User B Wallet"]
    H --> I["Transfer User B Amount<br/>to User A Wallet"]
    
    I --> J{Both Transfers<br/>Successful?}
    
    J -->|Yes| K["✅ Mark as EXECUTED<br/>Generate txHash"]
    J -->|No| L["❌ Refund Both Users<br/>Mark as FAILED"]
    
    K --> M["Settlement Complete"]
    L --> M
    C --> N["Refund Complete"]
    
    N --> M
    
    style A fill:#fff3e0
    style G fill:#c8e6c9
    style K fill:#81c784
    style C fill:#ffcdd2
    style M fill:#b3e5fc
```

## 📱 User Journey Timeline

```mermaid
timeline
    title Atomic Swap User Journey (End-to-End)
    
    section User Creates Intent
        T+0m: User opens OTC page
        T+1m: Selects amount & chain
        T+2m: Signs with Xverse wallet
        T+3m: Intent submitted to backend
    
    section Matching Phase
        T+5m: Backend validates intent
        T+10m: Matching engine searches
        T+15m: Counterparty found!
        T+16m: Match notification sent
    
    section Signature Phase
        T+20m: User A signs match
        T+25m: User B signs match
        T+26m: Both signatures verified
    
    section Escrow Funding
        T+30m: User A funds escrow
        T+35m: User B funds escrow
        T+36m: Both amounts locked
    
    section Settlement
        T+40m: Settlement begins
        T+41m: Fund transfer execution
        T+42m: Confirmation received
        T+43m: Swap completed! ✅
```

---

## 🔗 Real Production Contracts (Deployed)

| Contract | Address | Purpose |
|----------|---------|---------|
| **Escrow** | `0x04ab814de65d6ce99bb8801b33fcd751ae21b28f1f4726363be93d12b99ecbf4` | Manages deposits & atomic swaps |
| **Liquidity Pool** | `0x0150909eba659342d7da0c11e0cae0234306b2c3d30abc611590d507ce678ef6` | Liquidity reserves |
| **Buy STRK** | `0x076ee99ed6b1198a683b1a3bdccd7701870c79db32153d07e8e718267385f64b` | BTC → STRK execution |
| **Sell STRK** | `0x0282fc99f24ec08d9ad5d23f36871292b91e7f9b75c1a56e08604549d9402325` | STRK → BTC execution |
| **STRK Token** | `0x04718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d` | Native token |

**Block Explorers**:
- 🔸 Bitcoin: https://mempool.space/testnet4
- 🔵 Starknet: https://sepolia.starkscan.co

## 🛠️ Tech Stack

| Layer | Technology |
|-------|-----------|
| **Frontend** | Next.js 14, React 18, TypeScript, Tailwind CSS |
| **Backend** | Next.js API Routes, Node.js |
| **Blockchain** | Cairo, Starknet, Scarb |
| **Wallets** | sats-connect (Bitcoin), starknet.js (Starknet) |
| **Oracle** | Pyth Network (Prices), Mempool API (BTC) |
| **ZK Proofs** | Garaga Verifier |
| **Database** | In-memory (demo), PostgreSQL (production) |

## 📋 Quick Start

### Prerequisites
- Node.js 18+
- Git
- Bitcoin Wallet: [Xverse](https://www.xverse.app/) or [Unisat](https://unisat.io/)
- Starknet Wallet: [Argent X](https://www.argent.xyz/download/) or [Braavos](https://www.braavos.app/)
- Bitcoin Testnet4 & Starknet Sepolia testnet tokens

### Installation

```bash
# 1. Clone repository
git clone https://github.com/AZAR2305/shadowflow-btc.git
cd shadowflow-btc

# 2. Install dependencies
npm install

# 3. Configure environment
cp .env.example .env.local
# Edit .env.local with configuration below

# 4. Start development server
npm run dev

# 5. Open browser
# http://localhost:3000
```

### Configuration

**Required in `.env.local`:**

```env
# Network
NEXT_PUBLIC_STARKNET_NETWORK=sepolia
NEXT_PUBLIC_STARKNET_RPC_URL=https://api.cartridge.gg/x/starknet/sepolia

# Smart Contracts  
NEXT_PUBLIC_ESCROW_CONTRACT_ADDRESS=0x04ab814de65d6ce99bb8801b33fcd751ae21b28f1f4726363be93d12b99ecbf4
NEXT_PUBLIC_LIQUIDITY_POOL_ADDRESS=0x0150909eba659342d7da0c11e0cae0234306b2c3d30abc611590d507ce678ef6

# Bitcoin
NEXT_PUBLIC_BTC_NETWORK=testnet4
NEXT_PUBLIC_BTC_RPC_URL=https://mempool.space/testnet4/api

# Starknet Executor (server-side only)
STARKNET_EXECUTOR_ADDRESS=0x731ce505c05b6ebb89e07553c6d2d38ec1d6672dd217e7af4e2f8261fe0274e
STARKNET_EXECUTOR_PRIVATE_KEY=<your_private_key_here>
```

### Running Locally

```bash
# Development (with hot reload)
npm run dev

# Production build
npm run build
npm start

# Type checking
npm run type-check

# Linting
npm run lint
```

Open [http://localhost:3000](http://localhost:3000) in your browser.

## 🔄 How It Works: End-to-End Flow